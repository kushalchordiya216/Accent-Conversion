...
# <TODO> Figure out where and when to use Batch Norm
...
class ANNModel(object):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        inputs_batch = np.reshape(inputs_batch, (-1, 30500))
        feed_dict = {
    ...
    def add_prediction_op(self):
        ...
        W5 = tf.get_variable("W5", shape=(self.config.state_size_4, self.config.n_classes), initializer=xavier)
        b5 = tf.get_variable("b5", shape=(1, self.config.n_classes))

        h1 = tf.nn.relu(tf.matmul(self.input_placeholder, W1) + b1)
        # h1 = tf.keras.layers.BatchNormalization(h1)
        # <TODO> BatchNorm might Help
        l1 = tf.nn.dropout(h1, keep_prob=1 - self.config.dropout_rate)
        h2 = tf.nn.relu(tf.matmul(l1, W2) + b2)
        # h2 = tf.keras.layers.BatchNormalization(h2)
        l2 = tf.nn.dropout(h2, keep_prob=1 - self.config.dropout_rate)
        h3 = tf.nn.relu(tf.matmul(l2, W3) + b3)
        # h3 = tf.keras.layers.BatchNormalization(h3)
        l3 = tf.nn.dropout(h3, keep_prob=1 - self.config.dropout_rate)
        h4 = tf.nn.relu(tf.matmul(l3, W4) + b4)
        pred = tf.matmul(h4, W5) + b5
        return pred
        # <TODO> Try Adding L2 regularization to reduce variance
    ...
    def optimize(self, sess, inputs, labels, test_inputs, test_labels):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            input_ = np.reshape(input_, (-1, 30500))
            predictions = sess.run(tf.argmax(self.pred, axis=1), feed_dict={self.input_placeholder: input_})
            correct = 0
            encountered = len(predictions)
            for i in range(0, len(predictions)):
                if predictions[i] == labels[i]:
                    correct += 1
            print('train Accuracy', correct / float(encountered))
            #<TODO> Make a smaller subset of training data to use for prediction this thing takes too long
            test_input = np.array(test_inputs)
            test_input = np.reshape(test_input, (-1, 30500))
            predictions = sess.run(tf.argmax(self.pred, axis=1), feed_dict={self.input_placeholder: test_input})
            correct = 0
            encountered = len(predictions)
            for i in range(0, len(predictions)):
                if predictions[i] == test_labels[i]:
                    correct += 1
            print('test Accuracy', correct / float(encountered))
        return losses
    ...
    def build(self):
        #self.mfcc = None  # Add a handle to this so we can set it later TODO figure out what this was supposed to mean
            ...
            for i in range(51, 594):
                if i < 10:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_a000' + str(i) + '.wav')
                elif 9 < i < 100:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_a00' + str(i) + '.wav')
                else:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_a0' + str(i) + '.wav')
                source_mfcc = np.array(
                    mfcc(source_wav_data, samplerate=source_sample_rate, numcep=self.config.num_mfcc_coeffs,
                ...
                source_mfcc_padded, _ = pad_sequence(source_mfcc, self.config.max_num_frames)
                inputs.append(source_mfcc_padded)
                labels.append(j)
            for i in range(1, 540):
                if i < 10:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_b000' + str(i) + '.wav')
                elif 9 < i < 100:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_b00' + str(i) + '.wav')
                else:
                    (source_sample_rate, source_wav_data) = wav.read(
                        'cmu_us_' + name_dict[j] + '_arctic/wav/arctic_b0' + str(i) + '.wav')
                source_mfcc = np.array(
                    mfcc(source_wav_data, samplerate=source_sample_rate, numcep=self.config.num_mfcc_coeffs,
                         nfilt=self.config.num_filters, winlen=self.config.window_len, winstep=self.config.window_step))
                source_mfcc = source_mfcc / np.mean(source_mfcc, axis=0)
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        SOURCE_DIR = '../cmu_us_awb_arctic/wav/'
        TARGET_DIR = '../cmu_us_ksp_arctic/wav/'
